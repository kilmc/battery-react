...
export const baseLengths = (config) => ({
  ...
  return values.reduce((obj, value) => {
    ...
    obj[`${separator}${negativeIndicator}${value}${valueIndicator}`]
...
export const baseColors = (config) => (array, separator = config.colorValueSeparator) => {
...
export const baseIntegers = (config) => (arr, separator = config.integerValueSeparator) =>
  arr.reduce((xs,x) => {
    xs[`${separator}${x}`] = x
    return xs
  },{});
...
export const baseKeywords = (config) => (obj, separator = config.keywordValueSeparator) => {
...
export const baseCompile = (config) => (atoms) => {
  ...
  const atomicCSS = String(allAtoms.flatMap(atomTree).map(printAtom))
    .replace(/,(?=(?:"[^"]*"|\([^()]*\)|\[[^\[\]]*\]|\{[^{}]*}|[^"\[{}()\]])*$)/gm, '');
